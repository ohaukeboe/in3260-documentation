...
\label{sec:org5bc71bc}
In order to configure the bottlenecks, we change the capacity at Router 1 and Router 2. The throughput at the WiFi AP is set to the lowest value it supports, which is 54 Mbps. Depending on the desired bottleneck location, the capacity at Router 1 and Router 2 was adjusted either above or below this threshold. Table \ref{tab:orga50493e} lists capacity values used in each bottleneck scenario.

\begin{table}[htbp]
\caption{\label{tab:orga50493e}Configured capacity at the two routers in the different bottleneck configurations}
\centering
\begin{tabular}{lrr}
...
The \texttt{tc\_rate} values were aligned with those specified in \ref{tab:orga50493e}, and \texttt{tc\_delay} was consistently set to \texttt{10ms}, in accordance with the delay used for all tests. The \texttt{tc\_bsize} was configured to equal 1 BDP.
\subsection{Traffic generation and types}
\label{sec:orgc54a3c2}
...
\caption{\label{fig:org83de7cf}Screenshot of NETHINT}
\end{figure}
...
\caption{\label{orga65e38e}CDF of correlation coefficients of CDF}
\end{figure}
...
\caption{\label{orgcb34b99}CDF plots for each BDP configuration}
\end{figure}
